            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={`Fallback: ${formData.productName || "Product Name"} | Site Name`}
                  maxLength={200}
                  className="mt-1.5"
                />
                <p className={`text-xs mt-1 ${formData.seoTitle.length > 60 ? 'text-red-500' : 'text-gray-400'}`}>
                  {formData.seoTitle.length}/60 characters
                  {!formData.seoTitle && <span className="text-amber-500 ml-1">— will use product name</span>}
                </p>
              </div>
              <div>
                ...
                <Textarea
                  ...
                  placeholder={`Fallback: ${formData.shortDescription || "Product short description"}`}
                  maxLength={300}
                  rows={3}
                  className="mt-1.5"
                />
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={`Fallback: ${formData.productName || "Product"}, ${formData.category || "category"}`}
                  className="mt-1.5"
                />
                {!formData.seoKeywords && <p className="text-xs text-amber-500 mt-1">Will auto-generate from product name & category</p>}
              </div>
              <div>
                ...
                {(ogImagePreview || formData.ogImage) ? (
                  <div className="space-y-2">
                    ...
                    <div>
                      <input type="file" accept="image/*" id="product-og-upload" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setOgImageFile(f); setOgImagePreview(URL.createObjectURL(f)); } }} />
                      <label htmlFor="product-og-upload" className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs bg-white border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">Change Image</label>
                  ...
                  <div>
                    <input type="file" accept="image/*" id="product-og-upload" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) { setOgImageFile(f); setOgImagePreview(URL.createObjectURL(f)); } }} />
                    <label htmlFor="product-og-upload" className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs bg-[#26A8E0] text-white rounded-lg cursor-pointer hover:bg-[#1a8abf]">Upload OG Image</label>
                    <p className="text-xs text-amber-500 mt-1.5">Leave empty — will use product main image</p>
                  </div>
                )}
              </div>
            </div>
